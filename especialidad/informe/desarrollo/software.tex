...
\label{productor_kafka}
	...
	\emph{Thread} de Java. Al momento de instanciar un objeto de ésta clase, se debe enviar la
...
\label{topologia_storm}
	...
	La tolopología de Apache Storm diseñada, tiene como objetivo leer datos de
	Apache Kafka con un \emph{spout} y, un \emph{bolt} que recibirá esos datos
	imprimiendo en los registros el dato leido. Como trabajo futuro sería posible
	diseñar e implementar \emph{bolts} más complejos para guardar los datos en una
	base de datos, implementar agregación de datos para cálculo de promedios,
	etcétera \ref{trabjo_futuro}.
	...
	La clase \emph{Main} tiene como responsabilidad recibir los parámetros el modo
	de prueba de la topología, puede ser \emph{local} o \emph{cluster}. En el
	primero, la prueba se realizará en un entorno local de Apache Storm, el
	segundo, es para utilizar la topología en un entorno como el desarrollado en
	capítulos anteriores. También, recibe como parámetros, la dirección de Apache
	Zookeeper, que será utilizada para encontrar las particiones de Apache Kafka
	para leer los datos.
	...
	Dicha clase (\ref{storm_topology_main}), también es reponsable de armar el
	\emph{spout} que leerá los datos de Apache Kafka, instanciar un objeto de la
	...
	una tupla de datos, el mensaje leído de Apache Kafka, y registra su
	procesamiento utilizando un \emph{logger}. Luego, envía un mensaje de
	...
	objetivo representar al objeto que contiene las métricas recibidas en cada
